...
reiniciar_celda(CELDA *celdaptr){
		*celdaptr = 0x00;
}
...
celda_poner_valor(CELDA *celdaptr, uint8_t val){
    ...
    }else if(val==0){ //reset celda
      *celdaptr = 0x00;
    }else if(val == 3){ // caracter especial
      *celdaptr = 0x07;
    }
...
celda_negra(CELDA celda){
	return (celda & 0x03)== 0x02;
}

__inline static uint8_t
celda_especial(CELDA celda){
	return (celda & 0x07)== 0x07;
}
